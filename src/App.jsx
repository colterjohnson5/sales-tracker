  ...
  const DailyView = () => {
    ...
    return (
      <div className="space-y-4">
        ...
        {/* POLICIES WRITTEN - Main header */}
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide">Policies Written</h2>
            <div className="flex items-center gap-3">
              {streak > 0 && <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-100 text-orange-600"><span className="text-sm">🔥</span><span className="text-xs font-semibold">{streak}-Day Streak</span></div>}
              <span className="text-3xl font-bold text-gray-900 tabular-nums">{totalPolicies}</span>
            </div>
          </div>
          <div className="grid grid-cols-4 gap-3">
            {[{ l: 'Auto', f: 'policies_auto', c: '#3B82F6', bg: 'bg-blue-50' }, { l: 'Fire', f: 'policies_home', c: '#A855F7', bg: 'bg-purple-50' }, { l: 'Life', f: 'policies_life', c: '#10B981', bg: 'bg-emerald-50' }, { l: 'Health', f: 'policies_health', c: '#F59E0B', bg: 'bg-amber-50' }].map(({ l, f, c, bg }) => (
              <div key={f} className={`${bg} rounded-xl p-3 text-center`}>
                <p className="text-xs font-medium text-gray-500 uppercase mb-2">{l}</p>
                <div className="flex items-center justify-center gap-1">
                  <button onClick={() => incrementValue(f, -1)} className="w-8 h-8 rounded-lg bg-white border border-gray-200 hover:bg-gray-50 text-gray-500 text-sm flex items-center justify-center transition-colors">−</button>
                  <span className="w-10 text-2xl font-bold tabular-nums" style={{ color: c }}>{currentDay[f] || 0}</span>
                  <button onClick={() => incrementValue(f, 1)} className="w-8 h-8 rounded-lg bg-white border border-gray-200 hover:bg-gray-50 text-gray-500 text-sm flex items-center justify-center transition-colors">+</button>
                </div>
              </div>
            ))}
