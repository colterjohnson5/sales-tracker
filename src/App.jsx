  ...
  const CommunityLeaderboard = () => {
    ...
    const getData = () => {
      if (leaderboardType === 'my-agency') {
        // Filter to just this agency's producers from team data
        ...
        return teamMembers.map(m => {
          ...
          return {
            id: m.id,
            name: m.name,
            total_policies: (d.policies_auto || 0) + (d.policies_home || 0) + (d.policies_life || 0) + (d.policies_health || 0)
          };
    ...
    // Load my agency data when switching to that tab
    React.useEffect(() => {
      if (leaderboardType === 'my-agency' && teamMembers.length === 0) {
        loadTeamData(leaderboardPeriod === 'today' ? 'today' : leaderboardPeriod === 'week' ? 'week' : leaderboardPeriod === 'month' ? 'month' : 'year');
      }
    }, [leaderboardType]);
    ...
    return (
      <div className="space-y-4">
        ...
        {/* Main tabs: Community vs My Agency */}
        <div className="flex gap-1 p-1.5 bg-gray-100 rounded-xl">
          <button onClick={() => setLeaderboardType('agency')} className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-semibold ${leaderboardType === 'agency' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>Community</button>
          <button onClick={() => setLeaderboardType('my-agency')} className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-semibold ${leaderboardType === 'my-agency' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>My Agency</button>
        </div>
        
        {/* Sub-tabs for Community: Agency vs Producer */}
        {leaderboardType !== 'my-agency' && (
          <div className="flex gap-1 p-1.5 bg-gray-100 rounded-xl">
            <button onClick={() => setLeaderboardType('agency')} className={`flex-1 py-2 px-4 rounded-lg text-sm font-semibold ${leaderboardType === 'agency' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>Agency</button>
            <button onClick={() => setLeaderboardType('producer')} className={`flex-1 py-2 px-4 rounded-lg text-sm font-semibold ${leaderboardType === 'producer' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>Producer</button>
          </div>
        )}
        
        {/* Time period selector */}
        <div className="flex gap-1 p-1.5 bg-gray-100 rounded-xl">
          {['today', 'week', 'month', 'year'].map((id) => <button key={id} onClick={() => { setLeaderboardPeriod(id); if (isMyAgency) loadTeamData(id); }} className={`flex-1 py-2 px-3 rounded-lg text-sm font-semibold capitalize ${leaderboardPeriod === id ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>{id}</button>)}
        </div>
        
        {(loadingLeaderboard || (isMyAgency && loadingTeam)) ? <div className="text-center py-12 text-gray-400">Loading...</div> : data.length === 0 ? (
          <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 text-center">
            <h3 className="font-semibold text-gray-900 mb-2">No one yet</h3>
            <p className="text-sm text-gray-500 mb-4">{isMyAgency ? "No team activity yet" : (profile.show_on_leaderboard ? "Log activity to appear!" : "Opt-in to see community rankings")}</p>
            {!profile.show_on_leaderboard && isOwner && !isMyAgency && <button onClick={() => setShowSettingsModal(true)} className="px-6 py-3 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold shadow-lg">Join Community Leaderboard</button>}
          </div>
        ) : (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            {data.length >= 3 && (
