      ...
      <div className="space-y-4">
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-5 shadow-xl">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-medium text-slate-400 uppercase tracking-wide">Today's Wins</h2>
            {streak > 0 && <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-500/20 text-orange-400"><span className="text-sm">🔥</span><span className="text-xs font-semibold">{streak}-Day Streak</span></div>}
          </div>
          <div className="grid grid-cols-4 gap-4">
            <div className="text-center"><p className="text-3xl font-bold text-white tabular-nums">{totalPolicies}</p><p className="text-xs text-slate-400 mt-1">Sales</p></div>
            <div className="text-center border-l border-slate-700"><p className="text-3xl font-bold text-white tabular-nums">{hcRate}<span className="text-lg">%</span></p><p className="text-xs text-slate-400 mt-1">Close Rate</p></div>
            <div className="text-center border-l border-slate-700"><p className="text-3xl font-bold text-white tabular-nums">{hq}</p><p className="text-xs text-slate-400 mt-1">Quoted</p></div>
            <div className="text-center border-l border-slate-700"><p className="text-3xl font-bold text-white tabular-nums">{pv}</p><p className="text-xs text-slate-400 mt-1">Pivots</p></div>
          </div>
        </div>

        {/* FUNNEL */}
        <div>
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3 px-1">📊 Sales Funnel</h3>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wide flex items-center gap-2"><Icons.Dollar /> Policies Written</h3>
            <span className="text-2xl font-bold text-gray-900 tabular-nums">{totalPolicies}</span>
          </div>
          <div className="grid grid-cols-4 gap-3">
            {[{ l: 'Auto', f: 'policies_auto', c: '#3B82F6' }, { l: 'Home', f: 'policies_home', c: '#8B5CF6' }, { l: 'Life', f: 'policies_life', c: '#10B981' }, { l: 'Health', f: 'policies_health', c: '#F59E0B' }].map(({ l, f, c }) => (
              <div key={f} className="bg-gray-50 rounded-xl p-3 text-center">
                <p className="text-xs font-medium text-gray-400 uppercase mb-2">{l}</p>
                <div className="flex items-center justify-center gap-1">
                  <button onClick={() => incrementValue(f, -1)} className="w-7 h-7 rounded-lg bg-white border border-gray-200 text-gray-400 text-sm flex items-center justify-center">−</button>
                  <span className="w-8 text-xl font-bold tabular-nums" style={{ color: c }}>{currentDay[f] || 0}</span>
                  <button onClick={() => incrementValue(f, 1)} className="w-7 h-7 rounded-lg bg-white border border-gray-200 text-gray-400 text-sm flex items-center justify-center">+</button>
                </div>
              </div>
            ))}
          ...
          <>
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-5 shadow-xl">
              ...
              <div className="grid grid-cols-4 gap-2 pt-4 border-t border-slate-700">
                ...
                <div className="text-center"><p className="text-lg font-bold text-purple-400 tabular-nums">{totals.policies_home}</p><p className="text-xs text-slate-500">Home</p></div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-xs font-semibold text-gray-400 uppercase">
                    <tr><th className="px-4 py-3 text-left">Producer</th><th className="px-2 py-3 text-center">📞</th><th className="px-2 py-3 text-center">💬</th><th className="px-2 py-3 text-center">🎯</th><th className="px-2 py-3 text-center">✅</th><th className="px-3 py-3 text-center">Policies</th><th className="px-4 py-3 text-right">Total</th></tr>
                  </thead>
                      ...
                      return (
                        <tr key={m.id} className="hover:bg-gray-50">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${i === 0 ? 'bg-amber-100 text-amber-700' : i === 1 ? 'bg-gray-200 text-gray-600' : i === 2 ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500'}`}>{i + 1}</span>
                              <div><p className="font-medium text-gray-900">{m.name}</p>{hit330 && <span className="text-xs text-emerald-600 font-semibold">3-30 ✓</span>}</div>
                            </div>
                          </td>
                          <td className="px-2 py-3 text-center text-gray-600 tabular-nums">{m.data.calls || 0}</td>
                          <td className="px-2 py-3 text-center text-gray-600 tabular-nums">{m.data.households_quoted || 0}</td>
                          <td className="px-2 py-3 text-center text-gray-600 tabular-nums">{m.data.pivots || 0}</td>
                          <td className="px-2 py-3 text-center text-gray-600 tabular-nums">{m.data.households_closed || 0}</td>
                          <td className="px-3 py-3">
                            <div className="flex justify-center gap-1">
                              <span className="px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded text-xs font-medium">{m.data.policies_auto || 0}</span>
                              <span className="px-1.5 py-0.5 bg-purple-50 text-purple-600 rounded text-xs font-medium">{m.data.policies_home || 0}</span>
                              <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-600 rounded text-xs font-medium">{m.data.policies_life || 0}</span>
                              <span className="px-1.5 py-0.5 bg-amber-50 text-amber-600 rounded text-xs font-medium">{m.data.policies_health || 0}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3 text-right"><span className="text-lg font-bold text-gray-900 tabular-nums">{p}</span></td>
                        </tr>
                      );
                    })}
                  </tbody>
      ...
      <header className="bg-white/80 backdrop-blur-lg border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-lg mx-auto px-5 py-4">
          ...
          <div className="flex gap-1 p-1.5 bg-gray-100 rounded-xl">
            ...
            <button onClick={() => setMainView('leaderboard')} className={`py-2.5 px-4 rounded-lg text-sm font-semibold ${mainView === 'leaderboard' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>🏆</button>
          </div>
          {mainView === 'my-numbers' && (
