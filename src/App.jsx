  ...
  const DailyView = () => {
    ...
    return (
      <div className="space-y-4">
        ...
        {/* POLICIES WRITTEN - Clean and professional */}
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-semibold text-gray-900">Policies Written</h2>
            <span className="text-4xl font-bold text-gray-900 tabular-nums">{totalPolicies}</span>
          </div>
          <div className="grid grid-cols-4 gap-4">
            {[{ l: 'Auto', f: 'policies_auto', c: 'text-blue-600' }, { l: 'Fire', f: 'policies_home', c: 'text-purple-600' }, { l: 'Life', f: 'policies_life', c: 'text-emerald-600' }, { l: 'Health', f: 'policies_health', c: 'text-amber-600' }].map(({ l, f, c }) => (
              <div key={f} className="text-center">
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-3">{l}</p>
                <div className="flex items-center justify-center gap-2">
                  <button onClick={() => incrementValue(f, -1)} className="w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 text-lg font-medium flex items-center justify-center transition-colors">−</button>
                  <span className={`w-8 text-2xl font-bold tabular-nums ${c}`}>{currentDay[f] || 0}</span>
                  <button onClick={() => incrementValue(f, 1)} className="w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 text-lg font-medium flex items-center justify-center transition-colors">+</button>
                </div>
              </div>
            ))}
