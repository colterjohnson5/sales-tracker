...
const SalesTracker = ({ user, profile, onLogout, onProfileUpdate }) => {
  ...
  const [leaderboardType, setLeaderboardType] = useState('my-agency');
  ...
  const [teamWeekOffset, setTeamWeekOffset] = useState(0); // 0 = current week, -1 = last week, etc.
  ...
  const loadTeamData = async (period, weekOffset = 0) => {
    ...
    if (period === 'today') { startDate = getLocalDateString(now); endDate = startDate; }
      ...
      // Support navigating to past weeks with weekOffset
      const d = now.getDay();
      const mon = new Date(now);
      mon.setDate(now.getDate() - (d === 0 ? 6 : d - 1) + (weekOffset * 7));
      const sun = new Date(mon);
      sun.setDate(mon.getDate() + 6);
      startDate = getLocalDateString(mon);
      // Don't go past today for end date
      const endDateObj = sun > now && weekOffset === 0 ? now : sun;
  ...
  const loadLeaderboardData = async (period, type) => {
    if (type === 'my-agency') return; // My agency uses team data, not this function
  ...
  // Load league name on mount
  React.useEffect(() => {
    if (profile?.league_id) {
      supabase.from('leagues').select('name').eq('id', profile.league_id).single().then(({ data }) => {
  ...
  const joinLeague = async () => {
    ...
    const { data: league, error } = await supabase
      .from('leagues')
      .select('*')
      .eq('code', leagueCode.toUpperCase().trim())
      .single();
    
    if (error || !league) {
      setLeagueError('Invalid league code');
      setJoiningLeague(false);
      return;
    }
    
    // Update all agency members to this league
    await supabase
      .from('profiles')
    ...
    onProfileUpdate({ ...profile, league_id: league.id });
    setLeagueCode('');
    setJoiningLeague(false);
  };
  ...
  const leaveLeague = async () => {
    ...
    setLeagueName(null);
    onProfileUpdate({ ...profile, league_id: null });
  };
      ...
      <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {isOwner && (
          <div className="mb-4 p-4 bg-gray-50 rounded-2xl">
            <p className="font-medium text-gray-900 mb-1">League</p>
            {leagueName ? (
              <div>
                <div className="flex items-center justify-between">
                  <p className="text-sm text-emerald-600 font-semibold">{leagueName}</p>
                  <button onClick={leaveLeague} className="text-xs text-red-500 hover:text-red-700">Leave</button>
                </div>
              </div>
            ) : (
              <div>
                <p className="text-sm text-gray-500 mb-3">Join a league to compete regionally</p>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 rounded-lg border border-gray-200 text-sm uppercase tracking-widest font-mono text-center"
                  />
                  <button 
                    onClick={joinLeague}
                ...
                {leagueError && <p className="text-xs text-red-500 mt-2">{leagueError}</p>}
              </div>
            )}
          </div>
        )}

        {/* Show league for non-owners */}
        {!isOwner && leagueName && (
          <div className="mb-4 p-4 bg-gray-50 rounded-2xl">
            <p className="font-medium text-gray-900 mb-1">League</p>
            <p className="text-sm text-emerald-600 font-semibold">{leagueName}</p>
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
          <div><p className="font-medium text-gray-900">National Leaderboard</p><p className="text-sm text-gray-500">{isOwner ? 'Show agency publicly' : 'Controlled by owner'}</p></div>
          {isOwner ? (
  ...
  const DailyView = () => {
    ...
    return (
      <div className="space-y-4">
        ...
        {/* POLICIES WRITTEN - Dark with colored circles */}
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-6 shadow-xl">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-sm font-medium text-slate-400 uppercase tracking-wide">Policies Written</h2>
            <span className="text-5xl font-bold text-white tabular-nums">{totalPolicies}</span>
          </div>
          <div className="grid grid-cols-4 gap-4">
            {[{ l: 'Auto', f: 'policies_auto', ring: 'ring-blue-500', text: 'text-blue-400' }, { l: 'Fire', f: 'policies_home', ring: 'ring-purple-500', text: 'text-purple-400' }, { l: 'Life', f: 'policies_life', ring: 'ring-emerald-500', text: 'text-emerald-400' }, { l: 'Health', f: 'policies_health', ring: 'ring-amber-500', text: 'text-amber-400' }].map(({ l, f, ring, text }) => (
              <button key={f} onClick={() => incrementValue(f, 1)} onContextMenu={(e) => { e.preventDefault(); incrementValue(f, -1); }}
                className="flex flex-col items-center group">
                <div className={`w-16 h-16 rounded-full ring-4 ${ring} bg-slate-800 flex items-center justify-center mb-2 group-active:scale-95 transition-transform`}>
                  <span className={`text-2xl font-bold tabular-nums ${text}`}>{currentDay[f] || 0}</span>
                </div>
    ...
    return (
      <div className="space-y-4">
        <div className="flex gap-1 p-1.5 bg-gray-100 rounded-xl">
          {['today', 'week', 'month', 'year'].map((id) => <button key={id} onClick={() => { setTeamPeriod(id); setTeamWeekOffset(0); }} className={`flex-1 py-2.5 px-3 rounded-lg text-sm font-semibold capitalize ${teamPeriod === id ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>{id}</button>)}
        </div>
        ...
        {teamPeriod === 'week' && (
          <div className="flex items-center justify-between bg-white rounded-xl p-3 shadow-sm border border-gray-100">
            <button 
              onClick={() => { const newOffset = teamWeekOffset - 1; setTeamWeekOffset(newOffset); loadTeamData('week', newOffset); }}
              className="w-10 h-10 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600 flex items-center justify-center transition-colors"
            >
              <Icons.ChevronLeft />
            </button>
            <div className="text-center">
              <p className="font-semibold text-gray-900">
                {(() => {
                  ...
                  const d = now.getDay();
                  const mon = new Date(now);
                  mon.setDate(now.getDate() - (d === 0 ? 6 : d - 1) + (teamWeekOffset * 7));
                  const sun = new Date(mon);
                  sun.setDate(mon.getDate() + 6);
                  return `${mon.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} - ${sun.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`;
                })()}
              </p>
              {teamWeekOffset === 0 && <p className="text-xs text-emerald-600 font-medium">This Week</p>}
              {teamWeekOffset === -1 && <p className="text-xs text-gray-500">Last Week</p>}
              {teamWeekOffset < -1 && <p className="text-xs text-gray-500">{Math.abs(teamWeekOffset)} weeks ago</p>}
            </div>
            <button 
              onClick={() => { if (teamWeekOffset < 0) { const newOffset = teamWeekOffset + 1; setTeamWeekOffset(newOffset); loadTeamData('week', newOffset); } }}
              disabled={teamWeekOffset >= 0}
              className="w-10 h-10 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600 flex items-center justify-center transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            >
              <Icons.ChevronRight />
            </button>
          </div>
  ...
  const [leagueProducerLeaderboardData, setLeagueProducerLeaderboardData] = useState([]);

  const loadLeagueLeaderboard = async (period) => {
    ...
    // Get all profiles in this league who are opted in
    const { data: leagueProfiles } = await supabase.from('profiles').select('*').eq('league_id', profile.league_id).eq('show_on_leaderboard', true);
    if (!leagueProfiles?.length) { setLeagueLeaderboardData([]); setLeagueProducerLeaderboardData([]); setLoadingLeaderboard(false); return; }
    
    const { data: activityData } = await supabase.from('daily_activity').select('*').gte('date', startDate).lte('date', endDate).in('user_id', leagueProfiles.map(p => p.id));
    
    // Producer leaderboard
    const pData = leagueProfiles.map((p) => {
      const acts = activityData?.filter((a) => a.user_id === p.id) || [];
      const t = acts.reduce((acc, d) => { acc.policies_auto += d.policies_auto || 0; acc.policies_home += d.policies_home || 0; acc.policies_life += d.policies_life || 0; acc.policies_health += d.policies_health || 0; return acc; }, { policies_auto: 0, policies_home: 0, policies_life: 0, policies_health: 0 });
      return { id: p.id, name: p.name, agency_name: p.agency_name, total_policies: t.policies_auto + t.policies_home + t.policies_life + t.policies_health };
    });
    pData.sort((a, b) => b.total_policies - a.total_policies);
    setLeagueProducerLeaderboardData(pData);
    
    // Agency leaderboard
    const agencyMap = {};
    leagueProfiles.forEach((p) => { if (!agencyMap[p.agency_id]) agencyMap[p.agency_id] = { agency_id: p.agency_id, agency_name: p.agency_name, total_policies: 0 }; });
    activityData?.forEach((a) => { const p = leagueProfiles.find((x) => x.id === a.user_id); if (p && agencyMap[p.agency_id]) agencyMap[p.agency_id].total_policies += (a.policies_auto || 0) + (a.policies_home || 0) + (a.policies_life || 0) + (a.policies_health || 0); });
    const aData = Object.values(agencyMap); aData.sort((a, b) => b.total_policies - a.total_policies);
    setLeagueLeaderboardData(aData);
    
    setLoadingLeaderboard(false);
  };

  const CommunityLeaderboard = () => {
    const [leagueSubType, setLeagueSubType] = useState('agency');
    const [nationalSubType, setNationalSubType] = useState('agency');
    
    // Get the right data based on leaderboard type
    const getData = () => {
      if (leaderboardType === 'my-agency') {
        const periodData = leaderboardPeriod === 'today' ? teamData : leaderboardPeriod === 'week' ? teamWeeklyData : leaderboardPeriod === 'month' ? teamMonthlyData : teamYearlyData;
        return teamMembers.map(m => {
          const d = periodData.find(p => p.user_id === m.id) || {};
          return { id: m.id, name: m.name, total_policies: (d.policies_auto || 0) + (d.policies_home || 0) + (d.policies_life || 0) + (d.policies_health || 0) };
        }).sort((a, b) => b.total_policies - a.total_policies);
      }
      if (leaderboardType === 'league') {
        return leagueSubType === 'producer' ? leagueProducerLeaderboardData : leagueLeaderboardData;
      }
      return nationalSubType === 'producer' ? leaderboardData : agencyLeaderboardData;
    };
    
    const data = getData();
    const isMyAgency = leaderboardType === 'my-agency';
    const isLeague = leaderboardType === 'league';
    const isNational = leaderboardType === 'national';
    const currentSubType = isLeague ? leagueSubType : nationalSubType;
    
    // Load data when switching tabs - track what we've loaded to prevent loops
    const [lastLoaded, setLastLoaded] = useState('');

    React.useEffect(() => {
      const key = `${leaderboardType}-${leaderboardPeriod}-${nationalSubType}`;
      if (key === lastLoaded) return; // Already loaded this combo
      
      setLastLoaded(key);
      
      if (leaderboardType === 'my-agency') {
        loadTeamData(leaderboardPeriod);
      } else if (leaderboardType === 'league' && profile?.league_id) {
        loadLeagueLeaderboard(leaderboardPeriod);
      } else if (leaderboardType === 'national') {
        loadLeaderboardData(leaderboardPeriod, nationalSubType);
      }
    }, [leaderboardType, leaderboardPeriod, nationalSubType]);

    const isLoading = (isNational && loadingLeaderboard) || (isMyAgency && loadingTeam) || (isLeague && loadingLeaderboard);

    return (
      <div className="space-y-4">
        <div className="text-center py-4"><h2 className="text-2xl font-bold text-gray-900">Leaderboard</h2></div>
        
        {/* Main tabs: My Agency / League / National - ALWAYS clickable */}
        <div className="flex gap-1 p-1.5 bg-gray-100 rounded-xl">
          <button onClick={() => setLeaderboardType('my-agency')} className={`flex-1 py-2.5 px-2 rounded-lg text-sm font-semibold ${leaderboardType === 'my-agency' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>My Agency</button>
          {profile?.league_id && (
            <button onClick={() => setLeaderboardType('league')} className={`flex-1 py-2.5 px-2 rounded-lg text-sm font-semibold ${leaderboardType === 'league' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>{leagueName || 'League'}</button>
          )}
          <button onClick={() => setLeaderboardType('national')} className={`flex-1 py-2.5 px-2 rounded-lg text-sm font-semibold ${leaderboardType === 'national' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>National</button>
        </div>
        
        {/* Sub-tabs for League & National: Agency vs Producer */}
        {(isLeague || isNational) && (
          <div className="flex gap-1 p-1.5 bg-gray-100 rounded-xl">
            <button onClick={() => isLeague ? setLeagueSubType('agency') : setNationalSubType('agency')} className={`flex-1 py-2 px-4 rounded-lg text-sm font-semibold ${currentSubType === 'agency' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>Agency</button>
            <button onClick={() => isLeague ? setLeagueSubType('producer') : setNationalSubType('producer')} className={`flex-1 py-2 px-4 rounded-lg text-sm font-semibold ${currentSubType === 'producer' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>Producer</button>
          </div>
        )}
        
        {/* Time period selector */}
        <div className="flex gap-1 p-1.5 bg-gray-100 rounded-xl">
          {['today', 'week', 'month', 'year'].map((id) => (
            <button key={id} onClick={() => setLeaderboardPeriod(id)} className={`flex-1 py-2 px-3 rounded-lg text-sm font-semibold capitalize ${leaderboardPeriod === id ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}>{id}</button>
          ))}
        </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 text-center">
            <h3 className="font-semibold text-gray-900 mb-2">No one yet</h3>
            <p className="text-sm text-gray-500 mb-4">
              {isMyAgency ? "No team activity yet" : isLeague ? "No league activity yet" : (profile.show_on_leaderboard ? "Log activity to appear!" : "Opt-in to see national rankings")}
            </p>
            {!profile.show_on_leaderboard && isOwner && isNational && (
              <button onClick={() => setShowSettingsModal(true)} className="px-6 py-3 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold shadow-lg">Join National Leaderboard</button>
            )}
          </div>
        ) : (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            {data.length >= 3 && (
              <div className="bg-gradient-to-b from-blue-50 to-white p-6 border-b border-gray-100">
                <div className="flex items-end justify-center gap-6">
                  <div className="flex flex-col items-center">
                    <div className="w-14 h-14 rounded-full bg-gradient-to-br from-gray-300 to-gray-400 flex items-center justify-center shadow-lg mb-2"><span className="text-xl font-bold text-gray-600">2</span></div>
                    <p className="font-semibold text-gray-900 text-sm text-center">{(isMyAgency || currentSubType === 'producer') ? formatLeaderboardName(data[1].name) : data[1].agency_name}</p>
                    <p className="text-lg font-bold text-blue-600 mt-1">{data[1].total_policies}</p>
                  </div>
                  <div className="flex flex-col items-center -mt-4">
                    <div className="w-18 h-18 rounded-full bg-gradient-to-br from-yellow-300 to-yellow-500 flex items-center justify-center shadow-xl mb-2 ring-4 ring-yellow-200" style={{width:'72px',height:'72px'}}><span className="text-2xl font-bold text-yellow-700">1</span></div>
                    <p className="font-bold text-gray-900 text-center">{(isMyAgency || currentSubType === 'producer') ? formatLeaderboardName(data[0].name) : data[0].agency_name}</p>
                    <p className="text-xl font-bold text-blue-600 mt-1">{data[0].total_policies}</p>
                  </div>
                  <div className="flex flex-col items-center">
                    <div className="w-14 h-14 rounded-full bg-gradient-to-br from-amber-500 to-amber-700 flex items-center justify-center shadow-lg mb-2"><span className="text-xl font-bold text-amber-700">3</span></div>
                    <p className="font-semibold text-gray-900 text-sm text-center">{(isMyAgency || currentSubType === 'producer') ? formatLeaderboardName(data[2].name) : data[2].agency_name}</p>
                    <p className="text-lg font-bold text-blue-600 mt-1">{data[2].total_policies}</p>
                  </div>
                </div>
              </div>
            )}
            <div className="divide-y divide-gray-100">
              {data.slice(data.length >= 3 ? 3 : 0).map((item, i) => {
                ...
                const isMe = currentSubType === 'agency' ? item.agency_id === profile.agency_id : item.id === profile.id;
                return (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className={`font-medium ${isMe ? 'text-blue-600' : 'text-gray-900'}`}>
                          {(isMyAgency || currentSubType === 'producer') ? formatLeaderboardName(item.name) : item.agency_name}
                          {isMe && <span className="ml-2 text-xs">(You)</span>}
                        </p>
                        {currentSubType === 'producer' && !isMyAgency && <p className="text-xs text-gray-400">{item.agency_name}</p>}
                      </div>
                    </div>
        ...
        {/* Settings link for National */}
        {isNational && (
          <div className="bg-gray-50 rounded-xl p-4 flex items-center justify-between">
            <div><p className="text-sm font-medium text-gray-700">National Status</p><p className="text-xs text-gray-500">{profile.show_on_leaderboard ? 'Visible' : 'Hidden'}</p></div>
